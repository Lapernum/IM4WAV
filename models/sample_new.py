...
if __name__ == '__main__':
    ...
    if hps["CLIP_dir"]:
        ...
        # (When wav_per_object = 2)Trying to get two outputs from one class (pick two images from one class for the model to generate audio), even when there's only one image, it will duplicate the same image twice to get two wavs
        # Seems like we only need to modify this for loop to put all the segments into sequence for the model to generate audio
        # It simply put all the embeddings together and add index and class information for each embedding. We can do this by adding another for loop for segments. So it's like ClassLoop(SegmentsLoop() -> audio -> combine).
        for class_name in class_list:
            CLIP_class = CLIP[class_name]
            class_files = CLIP_class.keys()
            for class_file in class_files:
                image_features = CLIP_class[class_file]["image_features"]
                volume_factor = CLIP_class[class_file]["volume_factor"]
                file_images_num = len(volume_factor)
                chosen_objects["objects"] += ([class_file] * len(volume_factor))
                file_indices = list(range(file_images_num))
                class_clip_emb = [CLIP[class_name][class_file]["image_features"][file_indices[i]] for i in range(len(file_indices))]
                chosen_objects["indexs"] += file_indices
        ...
        for i in range(ys.shape[0]):
            # Combine the image class and the index
